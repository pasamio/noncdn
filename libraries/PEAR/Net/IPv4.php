...
class Net_IPv4
{
        ...
        if (strchr($address, "/")) {
             ...
             */
                ...
                if (! $myself->validateNetmask($parts[1])) {
                    throw new RuntimeException("invalid netmask value");
                }
    ...
    function calculate()
    {
        ...
        if (! is_a($this, "net_ipv4")) {
            $myself = new Net_IPv4;
            throw new RuntimeException("cannot calculate on uninstantiated Net_IPv4 class");
        }
        ...
        if (strlen($this->ip)) {
           ...
           throw new RuntimeException("ip address not specified");
        }
        ...
        if (strlen($this->bitmask)) {
            ...
            throw new RuntimeException("netmask or bitmask are required for calculation");
        }
	...
	function inSameSubnet($ip1, $ip2)
	{
		if (! is_object($ip1) || strcasecmp(get_class($ip1), 'net_ipv4') <> 0) {
			...
			if (PEAR::isError($ipobj)) {
				throw new RuntimeException("IP addresses must be an understood format or a Net_IPv4 object");
			}
		}
